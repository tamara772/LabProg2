/*Lista3 exercício 4. Ponteiros e strings.Faça um programa que:
...
str[strcspn(str, "\n")] = '\0';//enter finaliza a string então atribui o '\0' para a str
printf("String:  %s\n", str);
        ...
        for (int i = 0; vogais[i] != '\0'; i++) 
           {
            if (*point == vogais[i]) 
              {
                cont++;
                break;  //sair do laço e não ficar comparando com todas as vogais toda vez
              }
            }
        point++;  
       }
